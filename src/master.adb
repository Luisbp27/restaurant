    ...
    protected body ClientMonitor is
        ...
        begin
            ...
            for i in room_capacity'range loop
                ...
                if (room_type(i) = -1) or (room_type(i) = r_type and room_capacity(i) > 0) then
                    room := True;
                    n := i + 1;
                end if;
        ...
        begin
            ...
            if room > 0 then
                -- Update room type
                room_type(room_idx) := 1;

                -- Update the dynamic variables
                room_capacity(room_idx) := room_capacity(room_idx) - 1;

                -- Print
                capacity := room_capacity(room_idx);
            ...
            if room_capacity(room_idx) = 2 then
                room_type(room_idx) := 1;
            end if;
        ...
        begin
            ...
            if room_capacity(room_idx) = num_tables then
                room_type(room_idx) := -1;
            end if;

            -- Print
            capacity := room_capacity(room_idx);
            r_type := room_type(room_idx);
